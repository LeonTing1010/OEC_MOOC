...
<#if value??&&value.list??&&(value.list?size>0)>
...
<#if row_index==0>
     <tbody  class="${tbodyClass}"><tr onmouseover="this.bgColor='#eeeeee'" onmouseout="this.bgColor='#ffffff'">
     <#else>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td align="center" class="pn-sp">
							共 ${value.totalCount} 条&nbsp; 当前页
							<input type="text" value="<#if value.list?size < value.pageSize> ${value.list?size}<#else>${value.pageSize}</#if>"
							...
							<input class="first-page" type="button" value="首 页"
								onclick="_gotoPage('1');"
								<#if value.firstPage> disabled="disabled"</#if>/> 
							...
							<input class="next-page" type="button" value="下一页"
								...
								<#if value.lastPage> disabled="disabled"</#if>/> 
							<input class="last-page" type="button" value="尾 页"
								onclick="_gotoPage('${value.totalPage}');"
								<#if value.lastPage> disabled="disabled"</#if>/>&nbsp; 当前
						    	${value.pageNo}/${value.totalPage} 页 &nbsp;转到第
							...
							<input class="go" id="_goPage" type="button" value="转"
								...
								<#if value.totalPage==1> disabled="disabled"</#if>/> </td>
