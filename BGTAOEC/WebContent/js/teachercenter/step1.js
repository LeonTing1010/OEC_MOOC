...
function showJobList(){
	...
	if (isNull(html)) {
		$('#selectJob .list').load(base+"/teacherCenter/upload/step1_job/",function(){
...
function checkCourseName() {
	...
	if (!isNull(oldCourseName)&&(oldCourseName==courseName)) {
		return true;
	}
	var rtFalg = true;
	$.ajax({
		url:base+"/teacherCenter/checkCourse.ajax",
...
function changeIntroduction(){	
	...
	if (introduction.length>500) {
		...
		$('textarea[name=introduction]').val($('textarea[name=introduction]').val().substring(0,500));
		errorSpan('超出长度限制').appendTo($p);
		return false;
	}
}
function changeScoreMethod(){	
	...
	if (introduction.length>500) {
		$p.find('span').first().html('500/500');
		$('textarea[name=scoreMethod]').val($('textarea[name=scoreMethod]').val().substring(0,500));
		errorSpan('超出长度限制').appendTo($p);
		return false;
	}
}
...
//校验图片
function checkImg() {
	var titlePage =  $('input[name=titlePage]').val();	
    if (isNull(titlePage)) {
    	eoc.alert('提示','图片为空');
    	return false;
	}  
    if (!isNull(titlePage)&&titlePage.indexOf('jpg')==-1) {
    	eoc.alert('提示','图片格式必须为jpg');
    	return false;
	}
	return true;
}
...
function checkVideo() {
    ...
    if (!isNull(video)&&!((video.indexOf('flv')!=-1)||(video.indexOf('mp4')!=-1))) {
    	eoc.alert('提示','预览视频格式必须为flv或mp4');
    	return false;
	}
	return true;
}
...
function uploadImgOk(dataUrl, errorMsg,resourceName) {
	...
	if (isNull(errorMsg)) {
		...
		var $img = $('<img>').css("height","100px;").css("width","200px").attr("src",cache+dataUrl);
