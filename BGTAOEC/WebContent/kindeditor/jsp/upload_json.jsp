...
while (itr.hasNext()) {
	...
	if (!item.isFormField()) {
		...
		String timeStr= df.format(new Date()) + "_" + new Random().nextInt(1000);
		String newFileName = timeStr + "." + fileExt;
		String newSimpleName = timeStr + "_1." + fileExt;
		try{
			//原图 
			...
			//缩略图 -内容中使用
	        toSmaillImg(savePath+newFileName,savePath+"_"+newFileName);
	        
	        //缩略图 -问题与图片关系中使用
	        toSimpleImg(savePath+newFileName,savePath+"_"+newSimpleName);
		...
		obj.put("error", 0);
		obj.put("url", saveUrl +"_"+ newFileName);
...
private String getError(String message) {
 ...
 public static void toSmaillImg(String filePath,String thumbPath) throws Exception{  
        ...
        java.awt.Image bigJpg = javax.imageio.ImageIO.read(new java.io.File(filePath));  
	        ...
	        int new_w = 458;  
	        int new_h = 0;  
	        double tagsize = old_w/new_w;  
	        BigDecimal bg = new BigDecimal(tagsize);
        	tagsize = bg.setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
...
private static void toSimpleImg(String filePath,String thumbPath) throws Exception{  
        String newurl =thumbPath;  
        java.awt.Image bigJpg = javax.imageio.ImageIO.read(new java.io.File(filePath));  
